...
export function createControlPanel(params: Params, onChange: () => void) {
  ...
  withHelp(developer.add(params, 'devSphereMode', ['off', 'contain ball', 'radial shell', 'symmetric shell']).name('sphere mode').onChange(onChange), 'Constrains the developer knot toward a spherical envelope. Contain ball only prevents escape; radial shell pulls toward a round sphere; symmetric shell adds a low-amplitude symmetric shell modulation.');
  withHelp(developer.add(params, 'devSphereStrength', 0, 1, 0.001).name('sphere strength').onChange(onChange), 'Blend strength for the spherical envelope. Use 0 for raw knot validation and 1 for maximum shell conformance.');
  withHelp(developer.add(params, 'devSphereRadius', 0.8, 2.4, 0.005).name('sphere radius').onChange(onChange), 'Target radius of the developer-mode spherical envelope.');
  withHelp(developer.add(params, 'devSphereSymmetry', 3, 12, 1).name('sphere symmetry').onChange(onChange), 'Symmetry order used by the symmetric shell mode to distribute folds around the sphere.');
  withHelp(developer.add(params, 'devSelfAvoidance').name('self avoid').onChange(onChange), 'Keeps non-neighboring parts of the developer ribbon separated after knot morphing and spherical shell projection. This is a tube-thickness relaxation, not just a visual offset.');
  withHelp(developer.add(params, 'devSelfAvoidanceStrength', 0, 1, 0.01).name('avoid strength').onChange(onChange), 'How strongly the self-avoidance relaxation pushes apart distant parts of the ribbon when they get closer than the tube clearance.');
  withHelp(developer.add(params, 'devSelfAvoidanceIterations', 0, 10, 1).name('avoid passes').onFinishChange(onChange), 'Number of self-avoidance solver passes. More passes catch harder shell-compression conflicts but cost more CPU when the developer mesh rebuilds.');
  withHelp(developer.add(params, 'devTubeClearance', 1.2, 5.5, 0.05).name('tube clearance').onChange(onChange), 'Minimum centerline separation in multiples of ribbon width. Larger values make intersections less likely but may fight tight spherical packing.');
